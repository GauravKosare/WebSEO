...
import { generateSeoStrategy, AiUnavailableError } from "@/lib/ai/gemini";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const strategy = await generateSeoStrategy(parsedPage, page.finalUrl);

    scan.seoStrategy = strategy;
    await scan.save();
    ...
    return NextResponse.json({ error: "Could not generate a content strategy right now." }, { status: 502 });
  }
}
